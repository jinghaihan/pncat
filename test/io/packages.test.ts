...
import { createFixtureOptions, getSnapshotPath, toFixtureSnapshotPath } from '../_shared'
...
function normalizePackagePaths(packages: PackageMeta[]): unknown {
  return JSON.parse(JSON.stringify(packages, (key, value) => {
    if (key !== 'filepath' || typeof value !== 'string')
      return value
    return toFixtureSnapshotPath(value)
  }))
}
