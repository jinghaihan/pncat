...
export async function resolveRemove(args: string[], context: ResolveContext): Promise<ResolveResult> {
  ...
  const { deps, isRecursive = options.recursive } = parsePnpmOptions(args)
  if (!deps.length) {
    p.outro(c.red('no dependencies provided, aborting'))
    process.exit(1)
  }
  ...
  for (const dep of deps) {
    ...
    catalogPkgs.forEach((catalog) => {
      const rawDep = pnpmCatalogManager.getCatalogDep(dep, catalog)
      if (rawDep) {
        dependencies.push(rawDep)
        pnpmCatalogManager.removePackageDep(dep, rawDep.catalogName, updatedPackages)
      }
    })
  }

  if (removeDeps.length) {
    p.outro(c.yellow(`${removeDeps.join(', ')} is not used in any catalog`))
    await runPnpmRemove(removeDeps, { cwd: pnpmCatalogManager.getCwd(), recursive: isRecursive })
  }
